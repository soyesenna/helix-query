...
public class QueryChain<T, Q extends EntityPathBase<T>> {
  ...
  public <V> QueryChain<T, Q> whereBeforeNow(Field<LocalDateTime, Q> field) {
    return whereLessThan(field, LocalDateTime.now());
  }

  public <V> QueryChain<T, Q> whereAfterNow(Field<LocalDateTime, Q> field) {
    return whereGreaterThan(field, LocalDateTime.now());
  }
  ...
  /**
   * Apply Spring Data Pageable to the query. Sort resolution is delegated to a property->Field
   * mapper.
   */
